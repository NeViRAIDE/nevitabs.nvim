...
M.bufilter = function()
  ...
  for i, nr in ipairs(bufs) do
    if not vim.api.nvim_buf_is_valid(nr) then table.remove(bufs, i) end
  end
...
M.tabuflineNext = function()
  ...
  vim.cmd(
    curbufIndex == #bufs and 'b' .. bufs[1] or 'b' .. bufs[curbufIndex + 1]
  )
end
...
M.tabuflinePrev = function()
  ...
  vim.cmd(
    curbufIndex == 1 and 'b' .. bufs[#bufs] or 'b' .. bufs[curbufIndex - 1]
  )
end
...
M.close_buffer = function(bufnr)
  if vim.bo.buftype == 'terminal' then
    ...
    elseif not vim.bo.buflisted then
      ...
      if
        vim.g.nv_previous_buf
        and vim.api.nvim_buf_is_valid(vim.g.nv_previous_buf)
      then
